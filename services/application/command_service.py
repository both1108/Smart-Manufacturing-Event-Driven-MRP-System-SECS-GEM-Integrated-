...
Moved here from services/query/ on 2026-05-04. Reads and writes are
now separated: this is a write (it appends HostCommand* events to
event_store) and lives under services/application/ alongside other
write-side services. The old import path still works via a shim left
at services/query/command_service.py — drop the shim once every
caller has migrated.
